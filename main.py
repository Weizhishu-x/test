...
def main(args):
    ...
    if args.DA_mode == 'uda':
        assert args.batch_size % 2 == 0, f'args.batch_size {args.batch_size} should be a multiple of 2'
        batch_sampler_train = torch.utils.data.BatchSampler(
            sampler_train, args.batch_size//2, drop_last=True)
        data_loader_train = DataLoader(dataset_train, batch_sampler=batch_sampler_train,
                                       collate_fn=DAOD.collate_fn, num_workers=args.num_workers,
                                       pin_memory=True)
    ...
    if args.eval:
        ...
        if coco_evaluator is not None and "bbox" in coco_evaluator.coco_eval:
            ...
            class_names = data_loader_val.dataset.coco.getCatIds()  # 获取类别 ID
            class_names = [data_loader_val.dataset.coco.loadCats(ids)[0]['name'] for ids in class_names]  # 获取类别名称

            # 打印每个类别的 AP
            print("\nPer-class AP@0.5:")
            for i, class_name in enumerate(class_names):
                ap_50 = coco_eval.eval['precision'][0, :, i, 0, -1].mean()  # 提取 AP@0.5
                print(f"{class_name}: {ap_50:.4f}")
        ...
        if coco_evaluator is not None:
            if "bbox" in coco_evaluator.coco_eval:
                ...
                if utils.is_main_process():
                    with (output_dir / 'ap_log.txt').open('a') as f:
                        f.write(f'epoch:{epoch}\n')
                        f.write(f'mAP:{mAP_50:.3f}\n')
                        for i, class_name in enumerate(class_names):
                            ap_50 = coco_evaluator.coco_eval["bbox"].eval['precision'][0, :, i, 0, -1].mean()  # 提取 AP@0.5
                            f.write(f"\t{class_name}: {ap_50:.3f}\n") 
            if args.output_dir :
                ...
                if mAP_50 > mAP:
                    ...
                    if utils.is_main_process():
                        with (output_dir / 'best_log.txt').open('w') as f_best:
                            f_best.write(f'best map: {mAP:.3f}, epoch: {epoch}\n')
                            for i, class_name in enumerate(class_names):
                                ap_50 = coco_evaluator.coco_eval["bbox"].eval['precision'][0, :, i, 0, -1].mean()  # 提取 AP@0.5
                                f_best.write(f"\t{class_name}: {ap_50:.3f}\n")
